...
import com.wallet.strategy.service.impl.okex.enums.ContentTypeEnum;
...
import com.wallet.strategy.service.impl.okex.utils.HmacSHA256Base64Utils;
import okhttp3.*;
import okio.Buffer;
...
public class APIHttpClientV5 {
    ...
    public APIHttpClientV5(final APIConfiguration config, final APICredentials credentials) {
        this.config = config;
...
//    ,String simulated
    private Headers headers(final Request request, final String timestamp) {
        ...
        if (StringUtils.isNotEmpty(this.credentials.getSecretKey())) {
            ...
            LOG.error("拼接密码:="+this.credentials.getPassphrase());
...
//            builder.add("x-simulated-trading","1");
    ...
    private void printRequest(final Request request, final String timestamp) {
       ...
       //拼接Url
        ...
        APIHttpClientV5.LOG.info(requestInfo.toString());
    }
}
