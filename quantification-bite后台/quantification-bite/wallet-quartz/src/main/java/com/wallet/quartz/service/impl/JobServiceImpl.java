...
import com.wallet.quartz.service.impl.SysJobServiceImpl;
import com.wallet.quartz.util.ScheduleUtils;
...
public class JobServiceImpl implements IJobService
{
    ...
    public int pauseJob(SysJob job) throws SchedulerException
    {
        ...
        int rows = sysJobService.updateJob(job);
        if (rows > 0)
        {
    ...
    public int deleteJob(SysJob job) throws SchedulerException
    {
        Long jobId = job.getJobId();
        String jobGroup = job.getJobGroup();
        int rows = sysJobService.deleteJobById(jobId);
        if (rows > 0)
        {
    ...
    {
        job.setStatus(ScheduleConstants.Status.PAUSE.getValue());
        int rows = sysJobService.insertJob(job);
        if (rows > 0)
        {
            ScheduleUtils.createScheduleJob(scheduler, job);
        }
